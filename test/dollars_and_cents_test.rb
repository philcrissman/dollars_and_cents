...
class DollarsAndCentsTest < ActiveSupport::TestCase
  ...
  test "should mass assign price" do
    @item = TestRecord.new(:price => 12.34)
    
    assert @item.save
    assert @item.reload.price_in_cents == 1234
  end
  
  test "should update price" do
    @item = TestRecord.new(:price => 12.34)
    @item.save
    assert @item.update_attributes({:price => 43.21})
    assert @item.reload.price_in_cents == 4321
  end
